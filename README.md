...
- f - fully connected layer. Must contain the "length" field that defines the number of its outputs. Must be the last one. For the last layer the length must coincide with the number of classes. May also contain the "droprate" field, that determines the probabilty of dropping the input elements. Should not be too large, otherwise it drops everything.
