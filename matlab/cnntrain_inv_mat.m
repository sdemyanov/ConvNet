...
dimens_num = layers{1}{end}.length;
...
for i = 1 : params.numepochs
  ...
  for j = 1 : numbatches
    ...
    % first pass    
    images = zeros([pixels_num dimens_num batchsize]);        
    for m = 1 : batchsize
    ...
    images_act = reshape(images, [layers{2}{1}.mapsize dimens_num batchsize]);
    ...
    images_der = reshape(layers{2}{1}.d, [pixels_num dimens_num batchsize]);
    for m = 1 : batchsize                    
